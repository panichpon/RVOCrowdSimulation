...
public class LargeSpace12 extends JPanel {
	/**
	 * 
	 */
...
/*		Simulator.instance.addAgent(new Vector2D(0, 200));
	...
	/*	for (int i = 0; i < 5; i++) {
		...
		for (int i = 0; i < 12; i++) {
			int agentGroup = (random2.nextInt(11 - (10)) + (10));
			System.out.println(agentGroup);
			int xpos = (random2.nextInt(100 - (-100)) + (-100));
			int ypos = (random2.nextInt(100 - (-100)) + (-100));
			for (int j = 0; j < agentGroup; j++) {
				Simulator.instance.addAgent(new Vector2D(xpos + i * 2.0, ypos + j * 2.0));
				goalsList.add(new Goal(Simulator.instance.getAgentPosition(agentNo)));
	...
	public static void main(String[] args) {
		...
		final LargeSpace12 blocks = new LargeSpace12();
