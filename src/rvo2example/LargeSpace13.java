...
public class LargeSpace13 extends JPanel {
	/**
	 * 
	 */
...
/*		Simulator.instance.addAgent(new Vector2D(0, 200));
	...
	/*	for (int i = 0; i < 5; i++) {
		...
		for (int i = 0; i <= 60; i++) {
			for (int j = 0; j < 100; j++) {
				Simulator.instance.addAgent(new Vector2D(-300 + i * 10.0, 300 + j * 10.0));
				goalsList.add(new Goal(new Vector2D(-300 + i * 10, -1500 + j * 10)));
				
				double rand = random2.nextDouble();
	...
	/*				Simulator.instance.setAgentNeighborDistance(agentNo, 13);
		...
		/*			Simulator.instance.setAgentNeighborDistance(agentNo, 15);
				...
				/*	Simulator.instance.setAgentNeighborDistance(agentNo, (30 + (rand * (13 - 30))));
					...
					Simulator.instance.setAgentMaxSpeed(agentNo, (1.25 + (rand * (0.5 - 1.25))));*/
					...
					Simulator.instance.setAgentTimeHorizonAgents(agentNo, (90 + (rand * (12 - 90))));
					Simulator.instance.setAgentRadius(agentNo, (1.6 + (rand * (0.4 - 1.6))));
					Simulator.instance.setAgentMaxSpeed(agentNo, (1 + (rand * (0.5 - 1))));
					goalsList.get(agentNo).setColor(Color.BLACK);
				...
				/*	Simulator.instance.setAgentNeighborDistance(agentNo, (15 + (rand * (13 - 15))));
					Simulator.instance.setAgentMaxNeighbors(agentNo, (int)(17 + (rand * (7 - 17))));
					Simulator.instance.setAgentTimeHorizonAgents(agentNo, (40 + (rand * (30 - 40))));
					Simulator.instance.setAgentRadius(agentNo, (1.1 + (rand * (0.4 - 1.1))));
					Simulator.instance.setAgentMaxSpeed(agentNo, (1.55 + (rand * (1.25 - 1.55))));*/
	...
	/*	for (int i = 0; i < 1; i++) {
			for (int j = 0; j < agentTest; j++) {
				Simulator.instance.addAgent(new Vector2D(-50 + j * 5.0, 100).scalarMultiply(2));
				goalsList.add(new Goal(new Vector2D(-50.0 + j * 5.0, -60).scalarMultiply(2)));
				goalsList.get(agentNo).setColor(getColor());*/
				
				//P
		/*		Simulator.instance.setAgentNeighborDistance(agentNo, 15);
				Simulator.instance.setAgentMaxNeighbors(agentNo, 40);
				Simulator.instance.setAgentTimeHorizonAgents(agentNo, 38);
				Simulator.instance.setAgentRadius(agentNo, 0.4);
				Simulator.instance.setAgentMaxSpeed(agentNo, 1.55);*/
				
				//E
		/*		Simulator.instance.setAgentNeighborDistance(agentNo, 15);
				Simulator.instance.setAgentMaxNeighbors(agentNo, 23);
				Simulator.instance.setAgentTimeHorizonAgents(agentNo, 32);
				Simulator.instance.setAgentRadius(agentNo, 0.4);
				Simulator.instance.setAgentMaxSpeed(agentNo, 1.55);*/
				...
				//N
		/*		Simulator.instance.setAgentNeighborDistance(agentNo, 15);
				Simulator.instance.setAgentMaxNeighbors(agentNo, 9);
				Simulator.instance.setAgentTimeHorizonAgents(agentNo, 29);
				Simulator.instance.setAgentRadius(agentNo, 1.6);
				Simulator.instance.setAgentMaxSpeed(agentNo, 1.25);*/
	...
	/*			agentNo++;
		...
		obstacle1.add(new Vector2D(-25.0, -205.0).scalarMultiply(2));
		obstacle1.add(new Vector2D(-20.0, -200.0).scalarMultiply(2));
		obstacle1.add(new Vector2D(-200.0, 205.0).scalarMultiply(2));
		obstacle1.add(new Vector2D(-205.0, 200.0).scalarMultiply(2));
		
		Simulator.instance.addObstacle(obstacle1);
		obsList.add(obstacle1);

		final List<Vector2D> obstacle2 = new ArrayList<>();
		obstacle2.add(new Vector2D(25.0, -205.0).scalarMultiply(2));
		obstacle2.add(new Vector2D(205.0, 200.0).scalarMultiply(2));
		obstacle2.add(new Vector2D(200.0, 205.0).scalarMultiply(2));
		obstacle2.add(new Vector2D(20.0, -200.0).scalarMultiply(2));
	...
	private void setPreferredVelocities() {
		...
		/*	int nearAgent = Simulator.instance.getAgentNumAgentNeighbors(agentNo);
			if(nearAgent > 0) {
				...
				double goalNX = agentPos.getX() + (-sDistance) * FastMath.cos(sAngle * FastMath.PI / 180);
				double goalNY = agentPos.getY() + (-sDistance) * FastMath.sin(sAngle * FastMath.PI / 180);
				Vector2D subGoal = null;
				
				if(goalsList.get(agentNo).getSubGoalSize() <= 0 || nearAgent > 3 ) {
					if(agentPos.distanceSq(goalsList.get(agentNo).getGlobalGoal()) > 500.0) {
						subGoal = new Vector2D(goalX, goalY);
					}
				} else {
					subGoal = new Vector2D(goalNX, goalNY);
				}
	...
	public static void main(String[] args) {
		...
		final LargeSpace13 blocks = new LargeSpace13();
		...
		do {
			...
			/*if(Simulator.instance.getGlobalTime() == 700.0) {
				System.out.println("sim step: " + Simulator.instance.getGlobalTime());
				break;
			}*/
